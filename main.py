...
def fetch_and_save_cards(queries, filename):
    ...
    combined_df.to_csv(f"data/{filename}", index=False)
    print(f"Saved {filename} with {len(combined_df)} cards.")

# Fetch and save vintage and modern cards
fetch_and_save_cards(vintage_queries, "vintage_pkmn_cards_feb2025.csv")
fetch_and_save_cards(modern_queries, "modern_pkmn_cards_feb2025.csv")
